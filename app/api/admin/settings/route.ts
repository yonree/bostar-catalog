...
export async function GET() {
  const item = await prisma.brandSetting.findUnique({ where: { id: settingId } });
  return NextResponse.json({ items: item ? [item] : [] });
}

export async function POST(request: Request) {
  const body = await request.json();
  const item = await prisma.brandSetting.upsert({
    where: { id: settingId },
    update: settingData(body),
    create: { id: settingId, ...settingData(body) },
  });
  return NextResponse.json({ success: true, message: '网站设置已保存。', item });
}
